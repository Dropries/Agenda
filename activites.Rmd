...
```{r message=FALSE, warning=FALSE, include=FALSE}
conf<-read_yaml("config.yaml",eval.expr=T)
...
```{r echo=FALSE}
formated %>% spread(Jour,Temps) %>% as_tibble()%>%knitr::kable()
```
...
```{r echo=FALSE}
formated %>% group_by(Activité) %>% summarise('Temps total' = sum(Temps)) %>% arrange(desc('Temps total'))
```
